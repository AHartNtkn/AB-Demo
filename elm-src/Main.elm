...
import GoogleAnalytics exposing (..)
...
initialModel = \_ ->
  ( {picture = [ [ Del True , Del True , Del True ] , [ Virt True , Nothing , Virt True ] , [ Nothing , Virt True ] ] }
  , Cmd.none )
...
type Msg
  = Add GraphNode GraphNode Int Int
...
update msg model =
  case msg of
    Add fr n i j -> 
      case fr of
        Del _ -> ( { picture = addNode i j n model.picture }
                 , googleAnalytics {msg = "red", value = "("++String.fromInt i++", "++String.fromInt j++")"} )
        Virt _ -> ( { picture = addNode i j n model.picture }
                  , googleAnalytics {msg = "green", value = "("++String.fromInt i++", "++String.fromInt j++")"})
        _ -> ( { picture = addNode i j n model.picture }, Cmd.none)
  ...
  case node of
    ...
    Del _ -> [
      circle
        ...
        , color "red"
        , fill "currentColor"
        , onClick (Add (Del True) (Virt True) i j)
        ]
        [  ] ]
    Virt _ -> [
      circle
        ...
        , color "green"
        , fill "currentColor"
        , onClick (Add (Virt True) (Del True) i j)
        ]
        [] ]
    _ -> [
      circle
